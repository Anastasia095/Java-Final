...
/*
 * - Final Project
 * - Robert Hagerty, Anastasia Tarasenko, Ramon Reyes, Aparna Thalamarla.
 * Optional
 *  */
			...
			case "2":
				...
				while(checkInt == 0) {
					...
					try {
						...
						if(gpa > 4.00 || gpa < 0.00) throw new Exception();
		        ...
		        if (response.compareToIgnoreCase("Y") == 0) {
		        	...
		        	for (Person staffPrint: personList) {
		        		if (staffPrint instanceof Staff) {
		        			writer.printf("\t%d. %s%n", count2, staffPrint.getfullName());
		        	...
		        	while (q == 0) {
		        		...
		        		if (sort.compareToIgnoreCase("1") == 0) {
		        			...
		        			int count3 = 1;
		        			for (Student studentt: studentList) {
		        				writer.printf("\t%d. %s%n", count3, studentt.getfullName());
		        				writer.printf("\tID: %s%n", studentt.getId());
		        				writer.printf("\tGpa: %.2f%n", studentt.getGpa());
		        				writer.printf("\tCredit hours: %d%n", studentt.getCreditHours());
		        				writer.println();
...
abstract class Person {
	...
	public int compareToByName(Person otherPerson) {
...
//		
	...
	@Override
    public int compareToByName(Person otherStudent) {
        // Compare students based on FullName
        return getfullName().compareTo(otherStudent.getfullName());
    }
